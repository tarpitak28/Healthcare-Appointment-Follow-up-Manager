...
1. **Google Gemini Generative AI (`llmService.js` & `postVisitGuardrail.js`)**:
   - Model: Google Gemini (`gemini-1.5-flash` / `gemini-2.0-flash`) via `@google/generative-ai` SDK.
   - Pre-visit analysis output format: JSON with `urgencyLevel`, `chiefComplaint`, `suggestedQuestions`.
   - Post-visit summary output format: Strict JSON contract (`{ summary, diagnosis[], medications[], tests[], followUp, warnings[] }`).
   - Zero-Hallucination Guardrail: Zod runtime schema validation (`PostVisitSummarySchema`) + source-grounding normalizer verifying all clinical entities against original doctor notes/prescriptions.
   - Robust fallback logic returns deterministic default data when API key is unset, rate limited, or output grounding fails.
2. **Nodemailer, ICS Generator & Central Notification Service (`emailService.js`, `calendarService.js`, `notificationService.js`)**:
   - Central Notification Service (`notificationService.js`): Persists every system event to PostgreSQL `NotificationLog` (`BOOKING_CONFIRMATION`, `APPOINTMENT_CANCELLATION`, `DOCTOR_LEAVE_CANCELLATION`, `POST_VISIT_SUMMARY`, `MEDICATION_REMINDER`).
   - Idempotency: Unique `eventKey` index prevents duplicate notification creation across concurrent requests.
   - Fault Isolation: Email delivery failures are caught cleanly; business transactions (booking, cancellation, consultation completion) always succeed.
   - Bounded Exponential Retries: `cronService.js` processes overdue retries every minute up to 5 max attempts (+1m, +5m, +15m, +60m).
   - Generates standard iCalendar format (`BEGIN:VCALENDAR ... END:VCALENDAR`) with unique UIDs attached to booking confirmation emails.
