...
| **AI Pre-Visit Symptom Triaging** | [llmService.js](file:///e:/Health_Appointment/server/src/services/llmService.js#L10-L58) | Gemini Flash API prompt extraction of `urgencyLevel` (`LOW`, `MEDIUM`, `HIGH`), chief complaint, and 3 suggested doctor questions with offline fallback defaults. | **VERIFIED WORKING** |
| **AI Post-Visit Summary Zero-Hallucination Guardrail** | [llmService.js](file:///e:/Health_Appointment/server/src/services/llmService.js#L63-L140), [postVisitGuardrail.js](file:///e:/Health_Appointment/server/src/utils/postVisitGuardrail.js) | Gemini Flash API clinical notes summarizer enforced by Zod schema validation (`PostVisitSummarySchema`), source-grounding verification engine, and 11-case test suite (`test_post_visit_guardrail.js`). | **VERIFIED WORKING** |
| **Admin Doctor Management & Leave System** | [adminController.js](file:///e:/Health_Appointment/server/src/controllers/adminController.js#L27-L146) | - `POST /api/doctor/appointments/:appointmentId/post-visit`: Submits clinical notes and triggers Gemini 2.0 Flash JSON post-visit summary generation with Zod & source-grounding guardrails.
- `POST /api/doctor/appointments/:appointmentId/approve-summary`: Doctor endpoint to approve flagged AI post-visit summaries, clearing `needsHumanReview: false` with audit logging.
- `POST /api/doctor/appointments/:appointmentId/cancel`: Cancels an appointment from the doctor interface.
- `services/notificationService.js`: Central idempotent notification manager with PostgreSQL persistence (`NotificationLog`), atomic state transitions (`PENDING` -> `PROCESSING` -> `SENT`/`FAILED`), and exponential retry scheduling. | **VERIFIED WORKING** |
